void tube_setting(){
 if (!validateToken()) {
  ...
  html += "<label for=\"number\">Вес импульса счетчика 1 (литров на импульс):</label>";
  ...
  html += "<label for=\"number2\">Начальные показания счетчика 1:</label>";
  ...
  html += "<label for=\"number\">Вес импульса счетчика 2 (литров на импульс):</label>";
  html += "<input type=\"number\" id=\"number3\" name=\"counter_config2\" placeholder=\"Пример: 3200\" pattern=\"[0-9]{4,}\" value=\""+String(settings.counter_coeff_2)+"\" required><br><br>";
  html += "<label for=\"number2\">Начальные показания счетчика 2:</label>";
  html += "<input type=\"number\" id=\"number4\" name=\"counter_total2\" placeholder=\"Пример: 12345.67\" pattern=\"[0-9]+([\\.,][0-9]+)?\" step=\"0.01\" value=\""+String(settings.total_2)+"\" required><br><br>";  
