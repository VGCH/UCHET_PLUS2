void send_config(){
 if (!validateToken()) {
  ...
  html += "</head><body>";
  html += "<h2>Конфигурация передачи данных</h2>";
  html += "<form>";
  ...
  html += "<input type=\"checkbox\" id=\"switchpost\" "+post_act()+" ><text> Отправлять данные в личный кабинет на CYBEREX Tech </text>";
  html += "<div id=\"response3\">"+status_post_en()+"</div><br>";
  ...
  html +=" </form><br>";
  html += "<form id=\"config-form-post\" >";
  html += "<div id=\"response-post\">";
  html += "<label for=\"token\">Токен пользователя:</label>";
  html += "<input type=\"text\" id=\"token\" name=\"token\" placeholder=\"Токен пользователя из личного кабинета.\" value=\""+String(settings.post_token)+"\" required><br><br>";
  ...
  html += "<input type=\"text\" id=\"topic\" name=\"topicname\" placeholder=\"Пример: [имя_топика]/temp\"  value=\""+settings.mqtt_topic+"\" required><br><br>";
  ...
  html += "<form id=\"config-form-time\" >";
  html += "<div id=\"response-time\">";
  html += "<label for=\"number\">Периодичность отправки показаний(мин):</label>";
  html += "<input type=\"number\" id=\"number\" name=\"times\" placeholder=\"Пример: 24\" pattern=\"[0-9]{10,}\" value=\""+String(settings.mqtt_time)+"\" required><br><br>";
  html += "<input type=\"submit\" value=\"Сохранить\">";
  html += "</div>";
  html += "</form><br>";
...
String full_cong(){
    ...
    data += "<text> Периодичность обмена(ч): <b>"+String(settings.mqtt_time)+"</b></text><br>";
    return data;
}
