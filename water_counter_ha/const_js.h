...
const char html[] = R"(
                  ...
                  div#response2, div#response21, div#response3 {
                       margin: 10px auto 0;
                       max-width: 500px;
                       padding: 20px;
                       border-radius: 10px;
...
const char config_js[] = R"rawliteral(
  <script>
   ...
   <script>
        const form2 = document.getElementById('config-form-post');
              form2.addEventListener('submit', (event) => {
                    ...
                    const xhr = new XMLHttpRequest(); 
                          ...
                          xhr.addEventListener('load', () => {
                           if (xhr.status === 200) {
                                  document.getElementById('response-post').innerHTML = xhr.responseText;
                                    }else{
                                       document.getElementById('response-post').innerHTML = 'Ошибка отправки';
                                          }
                                    });
                          xhr.send(formData);
                       });
    ...
    <script>
        const form3 = document.getElementById('config-form-time');
              form3.addEventListener('submit', (event) => {
                    ...
                    const xhr = new XMLHttpRequest(); 
                          ...
                          xhr.addEventListener('load', () => {
                           if (xhr.status === 200) {
                                  document.getElementById('response-time').innerHTML = xhr.responseText;
                                    }else{
                                       document.getElementById('response-time').innerHTML = 'Ошибка отправки';
                                          }
                                    });
                          xhr.send(formData);
                       });
  ...
  <script>
          document.getElementById('switchpost').addEventListener('change', function() {
                  var status = this.checked ? 'on' : 'off';
                  var xhr = new XMLHttpRequest();
                  xhr.open('POST', '?page=indata', true);
                  xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
      ...
      function server_time(){
         var worktime = new XMLHttpRequest();
          ...
          if (worktime.readyState === 4 && worktime.status === 200) {
                ...
                document.getElementById("timew2").innerHTML   = obj.timew2;
                document.getElementById("Count1").innerHTML   = obj.Count1;
                document.getElementById("Count2").innerHTML   = obj.Count2;
                document.getElementById("Voltage").innerHTML  = obj.Voltage;
                document.getElementById("Count_1").innerHTML  = obj.Count_1;
                document.getElementById("Count_2").innerHTML  = obj.Count_2;
